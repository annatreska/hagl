...
// Grid thumbnails live in _strona_HaGL/miniatury/ (mirrored paths, square
// 600px, generated by scripts/make-thumbs.sh). Detail view keeps full images.
function thumbSrc(src) {
  return src.replace(`${ASSETS}/`, `${ASSETS}/miniatury/`);
}

// Attach smooth fade-in + graceful fallback to the full image if the
// thumbnail doesn't exist yet (e.g. new photo before running make-thumbs).
function prepareGridImg(img, fullSrc) {
  img.addEventListener('load', () => img.classList.add('loaded'), { once: true });
  img.addEventListener('error', () => {
    if (img.src !== fullSrc) img.src = fullSrc;
    else img.classList.add('loaded'); // both failed — unhide so alt text shows
  });
...
function makeEventGridItem(ev) {
  ...
  if (isVideo(ev.src)) {
    ...
    const img = document.createElement('img');
    img.src = thumbSrc(ev.src);
    img.alt = ev.label;
    img.loading = 'lazy';
    img.decoding = 'async';
    prepareGridImg(img, ev.src);
    div.appendChild(img);
  }
